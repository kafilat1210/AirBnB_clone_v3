...
class TestDBStorage(unittest.TestCase):
    ...
    def test_all_no_class(self):
		...
		all.objects = session.query(State).all()
    ...
    def test_save(self):
		...
		self.assertEqual(retrieved_state.id, new_state>id)
		self.assertEqual(retrieved_state.name, new_state.name)
		self.assertIsNotNone(retrieved-state)


	@unittest.skipIf(models.storage_t != 'db' , "not testing db storage")
	def test_get(self):
		"""Test Method for obtainng  an instance db storage)"""
		...
		storage.save()
		retrieved-state = storage.get(State, state_instance.id)
